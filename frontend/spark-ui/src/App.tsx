...
export default function App() {
  ...
  async function req<T>(path: string, method: "GET" | "POST", body?: unknown): Promise<T> {
    const res = await apiRequest<T>({ baseUrl, path, method, token, body });
    if (!res.ok) {
    ...
    <main className="app-shell">
      <div className="spark-page">
        <header className="spark-header">
          <h1>Infusion Platform Console (React / Spark-Parity)</h1>
          <p>Fully wired to current API routes with Spark-style cards, fields, actions, and status pills.</p>
        </header>
        ...
        <div className="spark-grid">
          ...
          <SparkCard title="Pump Sessions">
            ...
            >
              ...
              <div className="spark-row">
                ...
                <div className="spark-actions" style={{ alignItems: "end" }}>
                  <button className="spark-button" disabled={!!busy.sessions} type="submit">Start</button>
                  <button
                    className="spark-button secondary"
          ...
          <SparkCard title="Drug Library">
            <form
              className="spark-form"
              onSubmit={(e) => {
                e.preventDefault();
                run("drugs", () =>
                  req("/drug-library/", "POST", {
            ...
            >
              ...
              <div className="spark-row">
                <SparkField label="Entry ID (fetch)">
                  <input value={drugId} onChange={(e) => setDrugId(e.target.value)} />
                </SparkField>
                <div className="spark-actions" style={{ alignItems: "end" }}>
                  <button className="spark-button" disabled={!!busy.drugs} type="submit">Create</button>
                  <button
                    className="spark-button secondary"
                    disabled={!!busy.drugs}
                    type="button"
                    onClick={() => run("drugs", () => req(`/drug-library/${drugId}`, "GET"))}
                  >
                    Fetch
                  </button>
                </div>
